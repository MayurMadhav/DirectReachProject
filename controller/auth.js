...
exports.login = async (req, res, next) => {
  ...
  // db.query(
  //   "SELECT FirstName FROM influencer_signup WHERE email = ?",
  //   [email],
  //   async (error, results) => {
  //     if (error) {
  //       console.log(error);
...
//         console.log(id);
//         const token = jwt.sign({ id, username: results[0].email.split("@")[0] }, process.env.JWT_SECRET, {
...
exports.loginBusiness = async (req, res, next) => {
  ...
  db.query('SELECT * FROM business_signup WHERE c_email = ?', [c_email], async (error, results) => {
    ...
    if (error) {
      console.log(error);
      return res.status(500).render("loginBusiness", {
        message: "Server error",
      });
    }
    if (!results.length) {
    ...
    // 2) If everything ok, store user data in session
    req.session.c_email = c_email;
    next();
    console.log('Session:', req.session);
